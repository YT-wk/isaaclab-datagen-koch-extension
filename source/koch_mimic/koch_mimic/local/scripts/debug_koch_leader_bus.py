...
from koch_mimic.local.leader_arm.dynamixel import DynamixelMotorsBus, OperatingMode
from koch_mimic.local.leader_arm.motors_bus import Motor, MotorNormMode
from koch_mimic.shared.configuration import activate_runtime_config, get_config_section, option_was_provided
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    status_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    monitor_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    leader_configure_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    leader_action_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    leader_monitor_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    torque_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    led_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    mode_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
    ...
    nudge_parser.add_argument("--port", default=None, help="Serial port, for example COM3.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    default_port = str(get_config_section(config, "leader_arm", "serial_port", default="auto"))
    default_baudrate = int(get_config_section(config, "leader_arm", "baudrate", default=DEFAULT_BAUDRATE))
    default_ids = ",".join(
        str(value) for value in get_config_section(config, "leader_arm", "probe_ids", default=list(DEFAULT_IDS))
    )

    if hasattr(args, "port") and args.port is None:
        args.port = default_port
    if hasattr(args, "baudrate") and not option_was_provided(argv_list, "--baudrate"):
        args.baudrate = default_baudrate
    if hasattr(args, "ids") and not option_was_provided(argv_list, "--ids"):
    ...
    if args.command not in {"ports", "scan"} and not args.port:
        parser.error("Serial port is required. Set it with --port or in configs/local.user.local.yaml.")
