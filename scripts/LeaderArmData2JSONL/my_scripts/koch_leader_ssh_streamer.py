...
class KochLeaderUSBReader:
    ...
    def __init__(self, port: str | None, baudrate: int, motor_layout: tuple[tuple[str, int], ...]):
        self.port = port
    ...
    def _probe_port(self, port_name: str) -> int:
        ...
        try:
            ...
            for motor_id in DXL_IDS_TO_PROBE:
                try:
...
def build_session_start(session_id: str, port_name: str, baudrate: int) -> dict[str, Any]:
    """构建一条会话开始消息。
    ...
    return {
        ...
        "baudrate": baudrate,
        "joint_order": [name for name, _ in MOTORS],
    }
...
def build_frame(
    ...
    - 底层调试时往往还需要原始 ticks。
    """
    ordered_names = [name for name, _ in MOTORS]
    ordered_samples = [joint_positions[name] for name in ordered_names]
...
def main() -> int:
    ...
    local_log_path = Path(args.local_log).expanduser().resolve() if args.local_log else None

    reader = KochLeaderUSBReader(port=args.port, baudrate=args.baudrate, motor_layout=MOTORS)
    ...
    try:
        while True:
            ...
            frame = build_frame(session_id, sequence, selected_port, args.baudrate, joint_positions)
            frame_line = json.dumps(frame, ensure_ascii=False, separators=(",", ":")) + "\n"
            ...
            if now >= reconnect_deadline:
                try:
                    ...
                    if session_announcement_pending:
                        session_start = build_session_start(session_id, selected_port, args.baudrate)
                        session_line = json.dumps(session_start, ensure_ascii=False, separators=(",", ":")) + "\n"
